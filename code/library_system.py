...
class Reader():
    def __init__(self, reader_info):
        ...
        self.reader_history = None
        self.checked_book_number = None
        self._unreturned_record = None
        self.unreturned_book_number = None
        self.due_record = None
        self.due_count = None
        self.update_data()
    ...
    @staticmethod
    def insert_hitory_record(reader, book, action):
...
def book_info_entry_single(isbn):
    ...
    if (len(isbn) != 10 or not re.search(r'^(\d{9})(\d|X)$', isbn)) and (
            len(isbn) != 13 or not re.search(r'^(\d{12})(\d)$', isbn)):
            ...
            while isbn != "0":
                ...
                if book_info:
                    ...
                    if book_info["isbn"] in set(books_df["isbn"]):
                        books_df.loc[books_df[books_df["isbn"] == isbn].index, "total_number"] = book_info[
                            "total_number"]
                    else:
                        # 如果ISBN不存在，则插入新的信息
                        print(border2)
    ...
    while choice != "0":
        ...
        if choice == "1":
            ...
            print(("感谢使用此套图书管理系统！"
                   ...
                   "\n如需寻求帮助，请在邮件标题中注明【图书馆软件问题】，在邮件正文中说明所遇到的问题，"
                   "并将【library.log】、【借阅记录.xlsx】和【图书馆信息.xlsx】三个文件添加至附件一并发送，谢谢！"))
